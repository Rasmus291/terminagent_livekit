...
class RoomAudioRecorder:
    ...
    MONITOR_CHUNK_FRAMES = 10  # Alle N Frames an Monitor senden (~500ms bei 50ms Frames)
    ...
    async def _capture_loop(self, track: rtc.Track, frame_list: list[bytes], label: str):
        ...
        try:
            ...
            async for event in stream:
                ...
                # Live-Monitor: sammle Frames und sende gebündelt
    ...
    async def _send_to_monitor(self, track: str, pcm_data: bytes):
        ...
        try:
            ...
            pass  # Monitor ist optional, Fehler ignorieren
...
async def lavita_agent(ctx: JobContext):
    ...
    session = AgentSession(
        llm=google.realtime.RealtimeModel(
            ...
            language="de-DE",
            # Niedrige Start-Sensitivität verhindert dass Echo als Speech erkannt wird
        ...
        # VAD konservativ: nur klare Sprache erkennen, kein Echo
        vad=silero.VAD.load(
        ...
        # Gemini Realtime Turn Detection — lässt Gemini selbst entscheiden
        turn_handling={
        ...
        # Anti-Stotter: Unterbrechungen nur bei deutlicher User-Sprache
        min_interruption_duration=0.8,    # User muss min. 0.8s sprechen bevor Interrupt
        min_interruption_words=2,         # Mindestens 2 Wörter bevor Interrupt
        false_interruption_timeout=1.0,   # 1s Cooldown nach falschem Interrupt
    )
    ...
    await audio_recorder.notify_call_start(contact_name="")
